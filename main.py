...
class ArtifactGenerator:
    ...
    def generate_config_xml(self, output_file="out/config.xml"):
        ...
        if not self.classes or not self.root_class_name:  # Проверяем, что данные загружены и корневой класс определен
        ...
        root_element_name = self.root_class_name
        xml_content = f"<{root_element_name}>\n"
        root_class_data = self.classes[self.root_class_name]

        for attribute in root_class_data["attributes"]:
    ...
    def generate_meta_json(self, output_file="out/meta.json"):
        ...
        for class_name, class_data in self.classes.items():
            class_info = {  # Создаем словарь с информацией о классе
                "class": class_name,
            ...
            if min_multiplicity:
                class_info["min"] = min_multiplicity
            if max_multiplicity:
                class_info["max"] = max_multiplicity
...
def main():
    ...
    if not os.path.exists(default_config_file):
        config_file = input(
            f"Введите путь к config.json файлу (по умолчанию: {default_config_file}): ") or default_config_file
    else:
        config_file = default_config_file

    if not os.path.exists(default_patched_config_file):
        patched_config_file = input(
            f"Введите путь к patched_config.json файлу (по умолчанию: {default_patched_config_file}): ") or default_patched_config_file
    else:
